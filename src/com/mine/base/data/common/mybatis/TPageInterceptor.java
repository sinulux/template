...
public class TPageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if(obj instanceof Map){
			page=(PageMap)((Map) obj).get("pageMap");
		}else{
			page = (PageMap) ReflectUtil.getFieldValue(obj, "pageMap");
		}
		if (null != page) {
			if (page.getPageSize() <= 0||page.getStartNum()<0) {
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecord(Object obj,PageMap page, MappedStatement mappedStatement,
		...
		ReflectUtil.setFieldValue(boundSql, "sql",
				this.getCountSql(boundSql.getSql()));
		// 通过mappedStatement、参数对象page和BoundSql对象countBoundSql建立一个用于设定参数的ParameterHandler对象
		ParameterHandler parameterHandler = new DefaultParameterHandler(
				mappedStatement, obj, boundSql);
