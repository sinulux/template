...
Custom_table = function(){
	...
	this.openProgress = function() {
		if(this.extConfig.isOnloadProgress) {	// 是否显示进度条
			...
			customProgressbar.show({
				waitTime : 30*6000 ,	// 进度条的加载时间，单位秒，可以为空，默认超时60秒；
				barClosedFun : function() { // 进度条关闭后触发函数，默认空，自定义
					// 无记录
					var tableBody = '<tr><td colspan="' +_this.columnNum+ '" style="color: #FF0000;" class="gline" align="center">查询超时，请稍后再试……</td></tr>';
	...
	this.writeTable = function() {
		...
		if(null != this.pageData && null != this.pageData && this.pageData.length > 0 ) {
			...
			// 无记录
			tableBody = '<tr>'
					+ '<td colspan="' +this.columnNum+ '" style="color: red;width:'+this.table.width()+'px" class="gline" align="center">没有相关记录</td>'
					+ '</tr>';
			this.tableTbody.html(tableBody);
			// 初始化分页数据
			this.initTablePage();
